...
bowtie2 -p 8 -t -x $reference/human_index -1 $DIR/pair1.fq -2 $DIR/pair2.fq -S $DIR/alignment_human.sam
# delete intermediate large files to save some space
...
bowtie2 -p 8 -t -a --no-unal -x $reference/herv_index -1 $DIR/herv1.fq -2 $DIR/herv2.fq -S $DIR/alignment.sam
...
samtools flagstat $bam > $DIR/stats.txt
